...
/**
 * Delete the keyframe from channel
 * @param currentTime time at which we are removing the keyframe
 * @return boolean
 */
bool AnimChannel::DeleteKeyframe(int currentTime)
{
    for (auto keyframe: mKeyframes) {
        if (keyframe->GetFrame()==currentTime) {
            auto loc = std::find(std::begin(mKeyframes), std::end(mKeyframes), keyframe);
            if (loc!=end(mKeyframes)) {
